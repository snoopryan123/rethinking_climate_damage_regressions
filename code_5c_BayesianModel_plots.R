...
#####################
### Plot 3: marginal effects for the lag-growth specification (spec 4)
### (no AR coefficient plot for spec 4: useYearEffect = 0, phi is unused)
#####################
draws_lagGrowth <- readRDS("fullyBayesianModel_A_spec_4_lagGrowth.rds")

df_plot_ME_posteriorCI_lagGrowth <-
  draws_lagGrowth %>%
  select(ME_tx5d_t5, ME_tx5d_t25) %>%
  mutate(
    ME_tx5d_t5_ppPerSd  = ME_tx5d_t5  * sd_withinRegionTx5d,
...
plot_bayesianMarginalEffects_lagGrowth <-
  df_plot_ME_posteriorCI_lagGrowth %>%
  filter(str_detect(param, "ppPerSd")) %>%
  ggplot(aes(y = mean, x = method, color = T)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
...
ggsave("plots/plot_fullBayesianModelMarginalEffects_lagGrowth.png", width = 6, height = 6)
